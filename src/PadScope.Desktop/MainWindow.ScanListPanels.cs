...
public partial class MainWindow
{
    ...
    private enum ScanPanelMode { Normal, Collapsed, Expanded }
    private enum ScanPanelKind { Stages, Features }
    ...
    private void InstallScanListPanelControls()
    {
        ...
        if (_stagesPanel is null || _featureTestsPanel is null || _scanListHostGrid is null) return;
        ...
        if (_stagesHeader is null || _featureTestsHeader is null) return;

        AddPanelActions(_stagesHeader, StagesGrid, "test-stages",
            () => ToggleExpanded(ScanPanelKind.Stages),
            () => ToggleCollapsed(ScanPanelKind.Stages),
            out _stagesExpandButton, out _stagesCollapseButton);

        AddPanelActions(_featureTestsHeader, FeatureTestsGrid, "feature-tests",
            () => ToggleExpanded(ScanPanelKind.Features),
            () => ToggleCollapsed(ScanPanelKind.Features),
            out _featureExpandButton, out _featureCollapseButton);

        ApplyScanPanelLayout();
    }

    private static StackPanel? FindHeaderStack(Grid host, int row, string title) =>
        host.Children.OfType<StackPanel>().FirstOrDefault(panel =>
    ...
    private void AddPanelActions(
        StackPanel header, DataGrid grid, string exportStem,
        Action expandAction, Action collapseAction,
        out Button expandButton, out Button collapseButton)
    {
    ...
    private Button CreatePanelActionButton(string content, string toolTip) => new()
    {
        Content = content,
        Height = 28,
        MinWidth = 0,
    ...
    private void ToggleCollapsed(ScanPanelKind kind)
    {
        if (kind == ScanPanelKind.Stages)
        {
            ...
            if (_stagesMode == ScanPanelMode.Collapsed && _featureMode == ScanPanelMode.Expanded) _featureMode = ScanPanelMode.Normal;
        }
        else
        {
            _featureMode = _featureMode == ScanPanelMode.Collapsed ? ScanPanelMode.Normal : ScanPanelMode.Collapsed;
            if (_featureMode == ScanPanelMode.Collapsed && _stagesMode == ScanPanelMode.Expanded) _stagesMode = ScanPanelMode.Normal;
        }
        ApplyScanPanelLayout();
    }
    ...
    private void ApplyScanPanelLayout()
    {
        if (_scanListHostGrid is null || _stagesHeader is null || _featureTestsHeader is null ||
            _stagesPanel is null || _featureTestsPanel is null) return;
        ...
        else
        {
            ...
            _scanListHostGrid.RowDefinitions[0].Height = GridLength.Auto;
            _scanListHostGrid.RowDefinitions[1].Height = _stagesMode == ScanPanelMode.Collapsed ? new GridLength(0) : new GridLength(1, GridUnitType.Star);
            _scanListHostGrid.RowDefinitions[2].Height = GridLength.Auto;
            _scanListHostGrid.RowDefinitions[3].Height = _featureMode == ScanPanelMode.Collapsed ? new GridLength(0) : new GridLength(1, GridUnitType.Star);
        }

        if (_stagesExpandButton is not null) _stagesExpandButton.Content = stagesExpanded ? "Restore" : "Expand";
        if (_featureExpandButton is not null) _featureExpandButton.Content = featuresExpanded ? "Restore" : "Expand";
        if (_stagesCollapseButton is not null) _stagesCollapseButton.Content = _stagesMode == ScanPanelMode.Collapsed ? "Show" : "Collapse";
        if (_featureCollapseButton is not null) _featureCollapseButton.Content = _featureMode == ScanPanelMode.Collapsed ? "Show" : "Collapse";
    }
    ...
    private void ExportDataGrid(DataGrid grid, string exportStem)
    {
        try
        {
            List<ExportColumn> columns = grid.Columns.OfType<DataGridBoundColumn>()
                .Select(column => new ExportColumn(column.Header?.ToString() ?? "Column", (column.Binding as Binding)?.Path?.Path ?? string.Empty))
                .Where(column => !string.IsNullOrWhiteSpace(column.Path))
                .ToList();
            List<object> rows = grid.Items.Cast<object>().Where(item => item != CollectionView.NewItemPlaceholder).ToList();
        ...
        {
            MessageBox.Show(this, ex.Message, "PadScope export failed", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
    ...
    private static void ExportRowsAsCsv(string fileName, IReadOnlyList<object> rows, IReadOnlyList<ExportColumn> columns)
    {
        ...
        foreach (object row in rows)
            builder.AppendLine(string.Join(",", columns.Select(column => EscapeCsv(Convert.ToString(ReadPropertyPath(row, column.Path), CultureInfo.InvariantCulture) ?? string.Empty))));
        IOFile.WriteAllText(fileName, builder.ToString(), new UTF8Encoding(true));
    }

    private static void ExportRowsAsJson(string fileName, IReadOnlyList<object> rows, IReadOnlyList<ExportColumn> columns)
    {
        List<Dictionary<string, object?>> payload = rows.Select(row => columns.ToDictionary(
            column => column.Header,
            column => ReadPropertyPath(row, column.Path),
            StringComparer.OrdinalIgnoreCase)).ToList();
        IOFile.WriteAllText(fileName, JsonSerializer.Serialize(payload, new JsonSerializerOptions { WriteIndented = true }), new UTF8Encoding(false));
    }
