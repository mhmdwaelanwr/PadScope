...
public sealed class StickDiagnosticsAnalyzer
{
    ...
    public StickDiagnosticsAnalyzer(int rangeBinCount = DefaultRangeBinCount)
    {
        if (rangeBinCount < 8)
        {
            throw new ArgumentOutOfRangeException(nameof(rangeBinCount));
        }
    ...
    public void Add(float x, float y)
    {
        ...
        lock (_sync)
        {
            ...
            if (magnitude >= 0.18)
            {
                ...
                int bin = Math.Min(_rangeBins.Length - 1,
    ...
    public StickDiagnosticsSnapshot Snapshot()
    {
        ...
        {
            if (_sampleCount == 0)
                return new(0, 0, 0, 0, 0, 0, 0, 0, 0, _rangeBins.ToArray());
            ...
            double recommendedDeadzone = Math.Clamp(
                Math.Max(drift + 3 * jitter, _maximumMagnitude * 1.05), 0, 0.35);
            ...
            foreach (double radius in _rangeBins)
            {
                ...
                populated++;
                if (radius >= 0.90) fullRange++;
                circularityErrorSum += Math.Abs(Math.Clamp(radius, 0, 1.25) - 1.0);
            }

            double coverage = fullRange * 100.0 / _rangeBins.Length;
            double circularityError = populated == 0 ? 0 : circularityErrorSum / populated * 100.0;

            return new(_sampleCount, centerX, centerY, drift, jitter, _maximumMagnitude,
                recommendedDeadzone, coverage, circularityError, _rangeBins.ToArray());
        }
    }
    ...
    public void Reset()
    {
        ...
        {
            _sampleCount = 0;
            _sumX = _sumY = _sumX2 = _sumY2 = _maximumMagnitude = 0;
